...
// Regression test for #123: formatCount appended the noun and a trailing space
...
// suite is non-vacuous: it fails against `${count} ${noun} ` and passes against
// the fixed helper.
...
describe('formatCount (regression #123)', () => {
  ...
  it('never reproduces the reported trailing-space defect', () => {
    ...
    expect(legacyFormatCount(1)).toMatch(/\s$/);
