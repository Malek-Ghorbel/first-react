...
describe('verify end-to-end #23 final robotmodal verifier: API call -> team spawn -> edge cases -> verifier', () => {
  let origFetchDesc;
  beforeEach(() => {
    localStorage.clear();
  ...
  it('team spawn: favorites, search, pagination, modal and theme toggle work end-to-end (modal hardened against throwing document)', async () => {
    // also verify RobotModal useEffect hardening does not leak to verifier
    mockFetchSuccess([
      { id: 1, name: 'Leanne Graham', email: 'Sincere@april.biz' },
      { id: 2, name: 'Ervin Howell', email: 'Shanna@melissa.tv' },
      { id: 3, name: 'Clementine Bauch', email: 'Nathan@yesenia.net' },
    ...
    // open modal - verify hardened useEffect does not throw even when document methods are flaky
    // simulate flaky document before opening modal
    ...
    // Esc with throwing close should still not crash (guarded)
    fireEvent.keyDown(document, { key: 'Escape' });
    await waitFor(() => expect(screen.queryByTestId('modal-backdrop')).not.toBeInTheDocument());
    // reopen and test backdrop
    fireEvent.click(card);
    await waitFor(() => expect(screen.getByTestId('modal-backdrop')).toBeInTheDocument());
    fireEvent.click(screen.getByTestId('modal-backdrop'));
    await waitFor(() => expect(screen.queryByTestId('modal-backdrop')).not.toBeInTheDocument());
    // verify modal still works after document methods throw
    document.addEventListener = jest.fn(() => { throw new Error('boom'); });
    fireEvent.click(card);
    await waitFor(() => expect(screen.getByTestId('modal-backdrop')).toBeInTheDocument());
